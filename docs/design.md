...
1.  Stage 1.2's `unverified_relevant`/`missing_skill_candidates` (goal-text-derived) - tried *before* the `skill_gap_callback` human-ask; anything it resolves is removed from what the human would otherwise be asked about.
2.  Stage 2B, after the Architect's design is produced - re-runs the same extraction against the *design* text (not just the goal) and, for anything new the goal didn't mention, live-lookup-only (never a human-ask prompt at this later pipeline stage) - a vague goal ("build a message broker app") often only becomes concrete once the Architect makes real technology decisions.
