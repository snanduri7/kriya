...
Candidate Next Action: implement P2 (explicit approval/trust-manifest layer + CI non-interactive trust path) per the same-day DEEP design pass's Option E recommendation (field-level allowlist + provenance + digest-bound explicit trust), before proceeding to SEC-003/004/005 MCP containment implementation — the plugin vector this pass surfaced bypasses any MCP-specific containment entirely, so config-authority is the correct prerequisite ordering.
