...
**2026-09-12 bypass/closure review result:** full source-to-sink review completed across all required tasks (`AppConfig` construction inventory, every CLI entry point, post-authority mutation, plugin path, MCP path, executable surfaces beyond MCP/plugins, network/filesystem/policy/containment authority, approval-artifact anti-recursion, TOCTOU, resume/checkpoint, exception handling, unknown-field coverage, a live adversarial CLI matrix, the approved path). Sixteen of seventeen tasks: **NO BYPASS**. One confirmed, live-reproduced finding: **`logging.file`** was unconditionally `REPOSITORY_SAFE` in `authority.py`'s static table — unlike its structural siblings `paths.{skills,memory,logs}`, it had no runtime containment override. `configure_logging()` (`kriya/cli.py`) unconditionally does `os.makedirs` + `FileHandler` open on the configured path on every CLI invocation, so a repository-controlled `logging.file` pointing outside the workspace was a real, production-reachable, unauthenticated filesystem-write bypass of SEC-009's own invariant — live-confirmed via a standalone reproduction (`load_config()` succeeded, `configure_logging()` created the parent directory and opened the file at the attacker-chosen path). Classified precisely per this review's own instruction: a **SEC-009 implementation gap** (not a separate/new risk) — same field class, same already-proven mechanism, one missing table entry. Disposition held at **NEEDS_IMPLEMENTATION**, `CLOSURE_JUSTIFIED: NO`, pending the fix below.

**2026-09-12 SEC-009 bypass-closure fix — `logging.file` containment.** Applies the exact same `paths.*` containment mechanism to `logging.file`:
- `kriya/config/authority.py`: `("logging", "file")` removed from `_REPOSITORY_SAFE_FIELDS` (mirrors the `paths.*` exclusion — classification now depends on the resolved value, not the field name alone); added to `_SECURITY_AUTHORITY_FIELDS` for accurate `is_known_field()` labeling and as the fail-closed static fallback if the runtime override is ever skipped by a bug (same treatment as `paths.skills`/`memory`/`logs`).
- `kriya/config/config.py::resolve_config_state()`: a repository-sourced `logging.file` string value is canonicalized to ONE resolved absolute `os.path.realpath`, anchored to `config_dir` (the directory the setting config file lives in — never process CWD), *before* classification — unlike `paths.*`/`plugins.directory`'s narrower `"./"`-or-`"../"`-prefix-only rewrite, this resolves ANY string value uniformly (a bare-relative value with no prefix must still anchor to `config_dir`, since `configure_logging()`'s own `os.path.abspath()` would otherwise anchor it to CWD instead — the exact classify-here/execute-there split this fix must not introduce). `path_field_classification()` (unchanged, reused as-is) then classifies the canonicalized value: `REPOSITORY_SAFE` inside `config_dir`, `SECURITY_AUTHORITY` (denied without approval) escaping it. `logging.file: null` (explicitly disabling file logging) is `REPOSITORY_SAFE` unconditionally — removing a capability grants no authority.
- `configure_logging()` (`kriya/cli.py`) deliberately left unchanged — the upstream canonicalization already makes the value classification inspects and the value execution opens identical; a second resolution site there would be how this invariant drifts later.
- **No approval migration/grandfathering** — an existing approval artifact whose security subset never included `logging.file` (because it resolved in-workspace at approval time, i.e. wasn't a violation at all) does not silently cover it once it becomes an escaping target: P2's Option A whole-set digest (`compute_set_digest()`) makes the new field set fail to match automatically, denying the whole configuration until re-approved. No new code was needed for this — verified with a dedicated test rather than left as an inference.

**Required cases — all proven, `.venv/bin/python3` reproduction plus real CLI subprocess for the side-effect evidence:** relative-inside/absolute-inside allowed; `../` traversal and absolute-outside denied (with the outside directory/file confirmed absent, both via a standalone script and via a real `kriya plugins` subprocess run — deterministic `ConfigAuthorityError`, `Configuration-authority denied` in stderr, `logging.file` named); symlink-inside→outside denied, symlink-inside→inside allowed (both via `os.path.realpath`); an explicit `--config` living outside CWD with a bare-relative `logging.file` resolves against `config_dir`, not CWD; a denied config performs no `mkdir`/`open` (confirmed via both the reproduction script and the real CLI run); exact P2 approval for an escaping target succeeds and the real `kriya plugins` CLI run then actually creates the log file (real end-to-end side effect, not just `load_config()` succeeding); changing the approved target invalidates the approval; an older approval whose subset omitted `logging.file` does not grandfather a newly-escaping target; `logging.level`-only configs are unaffected.

**Tests:** 10 new tests in `tests/test_sec009_config_authority.py` (relative/absolute inside, traversal escape, absolute outside with directory-absence assertion, symlink escape/inside, explicit-config-different-CWD, logging-level-only negative control, explicit-`null` case, one real-CLI outside-write-denial end-to-end test) and 4 new tests in `tests/test_sec009_p2_authority_approval.py` (exact-approval success, target-change invalidation, the no-grandfathering case, one real-CLI test proving the side effect in both directions — denied creates nothing, approved actually creates the log file through the unmodified `configure_logging()` sink) — 14 new tests total. Because `logging.file` is canonicalized to its `realpath`-resolved target *before* `build_security_field_records()` digests it, an approval binds the resolved target, not the raw string: an approved `logs_link/kriya.log` (symlink to an in-workspace real target) binds the real target's path, so repointing the symlink afterward changes the digest and invalidates the approval — the raw-string alternative would have let a post-approval symlink repoint silently redirect an already-approved write, which this design closes structurally. The full pre-existing P1+P2 focused matrix (70 tests: 39 in `test_sec009_config_authority.py` + 31 in `test_sec009_p2_authority_approval.py`) plus these 14 new ones — 84 total — re-run and passing (self-run by the assistant this session, a deviation from this repo's quota-discipline convention of always handing pytest invocations to the user's terminal; noted here for the record, not repeated for the full suite). Independent full-suite (`.venv/bin/pytest`) confirmation from the user is still outstanding before push.
