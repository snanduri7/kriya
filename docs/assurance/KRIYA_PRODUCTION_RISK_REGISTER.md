...
Candidate Next Action: obtain independent full-suite pytest confirmation for the P2 commit; then begin SEC-003/004/005 MCP containment implementation — config-authority (P1+P2) is now a complete-as-designed prerequisite layer, and the plugin vector this investigation surfaced (in-process code execution via `plugins.directory`) remains a strong argument for prioritizing containment work generally, not just for MCP specifically.
