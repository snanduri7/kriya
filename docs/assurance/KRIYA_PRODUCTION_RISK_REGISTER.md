...
Language Scope: LANGUAGE_NEUTRAL · Deployment Relevance: REQUIRED · Effective Evidence Level: E2 (real deterministic tests, no live-model evidence required for this mechanism) · Disposition: **NEEDS_EVIDENCE** (still not CLOSED — `execution_policy.mode="enforce"` is now a selectable, tested config value as of P2, but no real `kriya generate`/`fix` project has actually run under it yet; independent-pytest and eventual live-config confirmation remain outstanding).
...
17 new/updated tests this pass (8 in `tests/test_tools.py` for GitTool/ShellTool, 3 in `tests/test_execution_policy_config.py`, 2 in `tests/test_config.py` for the config-mode change), self-verified via manual harness — all pass. A broader regression sweep across every P1+P2-touched test file plus `tests/test_run_ownership.py` (CONC-001) also self-verified clean: 103 passed / 0 failed / 15 skipped (pre-existing tests needing a real pytest `monkeypatch`/`CliRunner` fixture the manual harness can't provide). No live-model validation run or required. Exact pytest commands handed to the user for independent confirmation.

**Still outstanding, blocking CLOSED**: no real project has actually run `kriya generate`/`fix` with `execution_policy.mode="enforce"` set — only unit-level, directly-constructed-config evidence exists. Independent pytest confirmation of this pass's own claims is also still needed from the user before this register entry can be trusted at face value.
