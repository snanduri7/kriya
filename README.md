...
- **Live Lookup** (`autonomy.web_lookup_enabled`, **off by default** — the one opt-in exception to "zero cloud dependency"): auto-resolves a skill gap by searching a configured backend (e.g. a self-hosted SearXNG instance, `search.base_url`) instead of always waiting on a human to paste a URL. Search queries are built *exclusively* from bare technology-name strings already extracted by code (never goal text, design text, or your actual code) — a hard, code-enforced boundary, not a prompt instruction a model could ignore. Found references are always shown for a single batch confirmation before use. Tries several ranked results per term, not just the first — real testing showed a single top hit for a well-known library is often a landing page with nothing extractable, so one attempt isn't enough to call it "resolved." The query itself is `"{term} example"`, not `"{term} documentation"` — confirmed live against a real search backend that "documentation" consistently surfaces a project's landing/index page, while "example" surfaces genuinely extractable content (a GitHub config file, a quick-start code sample, a how-to page) for the same term.
- **Pre-Send Search Confirmation** (`autonomy.web_lookup_auto_approve`, **off by default**): a query's *content* is already hard-restricted (bare technology names only), but a separate gate controls *whether it's allowed to fire at all* — every outbound live-lookup search (goal-stage, design-stage, and the error-triggered retry-loop path alike) shows you the exact terms and target URL before it leaves the machine. Under `-y` with this still off, the query simply never fires, rather than the query going out anyway and its result being silently discarded — the actual pre-existing behavior this closes. Set `web_lookup_auto_approve: true` only if you've accepted unattended outbound search as part of your threat model.
- **Error-Triggered Live Lookup** (same `autonomy.web_lookup_enabled`/`search.base_url` switches, and now also gated by pre-send confirmation above): when a Quality Gates compile or Runtime Verification failure repeats identically on a second consecutive Developer retry attempt — a sign the model isn't self-correcting — Kriya tries a live lookup for the failure before the next retry, folding anything found directly into that retry's prompt. Query terms are restricted to Maven/Gradle-style `groupId:artifactId` coordinates found in the error text itself, never the raw error/stack-trace text (which can contain your own class/variable names) — the same hard query-safety boundary as goal/design-stage live lookup. The project's own coordinate (read fresh from the worktree's current `pom.xml` each retry, since the model can rename it mid-run) is always excluded, since Maven's own build banner prints it on every build and it would otherwise waste the one search Kriya gets per repeated failure. A second, separately-bounded pattern also catches wrong-import-path failures (e.g. importing a class from the wrong package within a library the project already depends on) - the symbol name only becomes a search term if it matches a class inside one of the project's own declared `pom.xml` dependencies, never an arbitrary symbol name. Ephemeral in what it produces: nothing is written to a skill's `rules.txt`, just folded into that one retry's prompt. A real live A/B test found this genuinely helps for actual unfamiliar-library knowledge gaps, but not for every repeated failure — many "whack-a-mole" retry failures turn out to be multi-file regeneration consistency issues rather than missing information, which this feature doesn't address (see docs/design.md §2.3.4 for detail).
- **Targeted Single-File Retry**: when a Quality Gates failure names one of your already-written files, Kriya's next Developer retry focuses on just that file instead of regenerating every file again — the target file(s) framed as the fix, every other file shown as read-only reference (its real current content, not just the error text — the full-set retry path never shows the model its own previous attempt at all). Runs on its own independent retry budget (3 attempts, separate from the normal 4+) and always uses your primary model, never the fallback chain — a model swap costs real time (measured 19-43s on Ollama), which would defeat the point of a fast, surgical fix. Identification is pure deterministic text matching against the error output, no LLM call — an error that doesn't name a known file (a bare exit code, a plugin-config error with no source file) simply falls back to today's full-file-set retry, exactly as before this feature existed. Every retry that's responding to a real prior failure also requires the model to write a short "FIX ANALYSIS" explaining the specific cause before it writes any code — found live that single-shot, non-reasoning completion (this project's own default local-model setup) will otherwise regenerate byte-for-byte identical broken code across every retry attempt, with the exact error present in every prompt and nothing forcing the model to actually engage with it. In a full-set retry this is scoped to only the file(s) the error actually names (not broadcast to every file being regenerated), paired with the exact broken source line extracted generically from the compiler's own file:line:col locator — present in every compile diagnostic shape, not tied to any specific error message.
...
- **Dependency Preservation Checklist**: when extending a project across milestones, a full-set `pom.xml` regeneration is explicitly told which existing Maven dependencies to preserve (found live: passive reference content alone wasn't enough to stop dependencies silently getting dropped across repeated retries) — and, separately, told to check the existing code for an already-working import before adding a *new* dependency for it. Found live on a second milestone: a working project had no explicit JMS dependency at all (it resolved transitively through an existing one), and the next milestone's generation added a redundant, nonexistent one anyway rather than checking what already worked — see `docs/design.md` §2.3.4h.
- **Retry Temperature & Lint Diagnostics** (both opt-in/automatic, research-backed): `llm.retry_temperature` lets you lower the sampling temperature specifically for a Developer completion that's fixing a real prior error (a cited study found code-gen success rate drops as temperature rises even at the low end of Kriya's default range — the opposite of "add randomness to a stuck retry"), left unset by default so it never silently changes behavior. Separately, Java compile checks now always pass `-Xlint:rawtypes,unchecked` to javac, so a raw-type mistake (a real, recurring cause of "incompatible types" failures) surfaces as a precisely-located warning right next to the resulting hard error, instead of a locationless one-line notice — see `docs/design.md` §2.3.4a.
