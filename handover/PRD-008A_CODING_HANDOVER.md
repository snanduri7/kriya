...
## A8 pytest round 1 (user, 2026-09-25)
Full suite at `ac43b57`: 5198 passed, 2 failed, 8 deselected, 144 warnings (1040.74 s). Both failures were in `tests/test_bootstrap_contract.py` (`test_production_code_has_no_undefined_names`, `test_every_production_annotation_resolves_at_runtime`), and both had the same real cause: `run_generation_workflow(work_unit: Optional["WorkUnitInvocation"])` was a forward reference with no binding in `kriya/workflow/workflow.py`. Fixed in `b99db27` by importing it at module level (plan_executor imports only control/execution_plan/plan_adapters, so there is no cycle). Checked with `typing.get_type_hints`, which now resolves the parameter, and `ruff --select F821 kriya` is clean. No behaviour change. Awaiting rerun.
