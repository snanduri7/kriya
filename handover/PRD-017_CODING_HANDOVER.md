...
## Decisions to review
1. **Unset fallback `max_tokens` now inherits the primary's.** `FallbackModelConfig.max_tokens` is `Optional[int] = None` (was `4096`). A chain entry without `max_tokens` gets `llm.max_tokens` on every call, for the Developer and for role escalation. Role chains without an explicit value move from 4096 to the primary's value; a role's own ceiling (`planner_max_tokens` etc.) still clamps it. An explicit value is always used. The alternative (keep 4096 and apply it to the Developer) would have cut every existing demo config's fallback output to 4096.
