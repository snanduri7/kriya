...
3. **A candidate that changes the toolchain declaration** makes PRE and POST not comparable under contained execution, and that blocks. An intentional toolchain migration therefore needs `disabled` (or a human decision), not a silent comparison across two toolchains.
4. **Production stays sealed to `required`**, which is stricter than `auto`.

## Tests (plain runner; you run pytest)
`tests/test_prd024_baseline_auto_policy.py`, 19 passed:
...
- a different environment is NOT_COMPARABLE and blocks;
