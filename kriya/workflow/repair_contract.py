...
def build_repair_contract(
    ...
    *,
    ...
    ambiguous-evidence case) and COORDINATED (this module). Adding a second
    evidenced obligation kind later means adding one more `elif
    obligation.kind is ObligationKind.X:` branch here, deriving that kind's
    own evidence with its own dedicated deterministic function - deliberately
    NOT a registry/plugin system (2026-08-29 design review: "no generalized
    obligation handler hierarchy... before we need it")."""
    if obligation.kind is not ObligationKind.PROCESS_BOUNDARY_COMPATIBILITY:
        return None
    if not evidence:
        return None
