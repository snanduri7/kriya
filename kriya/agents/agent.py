...
class RunVerifierAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "a run command from the generated files, set command_source to \"inferred\": a pom.xml "
            "whose exec-maven-plugin configuration has a <mainClass> element (with or without "
            "<jvmArguments>) implies [\"mvn\", \"exec:java\"] - this is the normal, most common "
            "exec-maven-plugin shape, since exec:java runs in Maven's own JVM using the "
            "already-resolved classpath and needs no separate process or explicit -classpath. Only "
            "infer [\"mvn\", \"exec:exec\"] when the configuration instead has an <executable> "
            "element or an <arguments> block explicitly passing a -classpath argument, since exec:exec "
            "spawns a separate process and is the less common shape. A Python file with a __main__ "
